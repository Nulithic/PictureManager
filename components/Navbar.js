...
export default function Navbar({ route, navigation, back }) {
  const path = usePath();
  ...
  const titleName = () => {
    if (path.pathList.length > 0) {
      const name = path.pathList[path.pathList.length - 1];
      if (name.length > 11) return name.substring(0, 11) + "...";
      else return name;
    } else return "Picture Manager";
  };
